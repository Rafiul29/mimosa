...
export const buttonVariants = cva("inline-block text-center px-6 py-3 rounded-xl border eq font-medium active:scale-105 disabled:border-gray disabled:bg-gray disabled:text-dark/50 disabled:cursor-not-allowed", {
  variants: {
    variant: {
      ...
      success: "border-green bg-green text-dark hover:border-green/90 hover:bg-green/90",
      ocean: "border-blue bg-blue text-dark hover:border-blue/90 hover:bg-blue/90",
      violet: "border-violet bg-violet text-white hover:border-violet/90 hover:bg-violet/90"
    },
    size: {
      auto: "w-auto",
      full: "w-full",
...
const Button: React.FC<ButtonProps> = ({
  ...
  return <button onClick={onClick} type={type} disabled={disabled} className={cn(buttonVariants({variant,size}))} >{children}</button>;
};
